...
- Utiliser la commande `ls` sans option ni argument. Que remarquez-vous ? le fichier dossier1 est apparu et est surligner en vert 
...
- Utiliser la commande `ls -F` . Que remarquez-vous ? jsp
- Réaliser deux copies de `fic1.txt` :
    - `fic2.txt` dans le répertoire `./Dossier1`
    - `fic3.txt` dans le répertoire `/workspaces/web2026`
- Renommer le fichier `fic1.txt` en `fichierNSI` . Commenter. j'ai deux copie du doc
- Utiliser la commande `tree` pour contrôler le résultat.
- Utiliser la commande `cd ../` puis `pwd` . Que remarquez-vous ? jsp
